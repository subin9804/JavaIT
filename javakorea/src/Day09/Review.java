...
public class Review {
	public static void main(String[] args) {
		...
		//정수형변수 = Integer.parseInt(sc.nextLine());		// 정수를 입력(nextInt 대체용. 정수형으로 형변환)
		//실수형변수 = Double.parseDouble(sc.nextLine()); 	// 입력받은 문자열을 실수로 변환(nextDouble 대체용)
		// 가능하면 nextLine 형변환해서 사용하는것이 좋음
		sc.close();
		
		// 자료형 변환
		// int <-> double: 기본자료형-괄호로 변환
		int num = 4;
		double dNum = 3.14;
		System.out.println((double)num);
		System.out.println((int)dNum);
		
		// String <-> int: 개발된 자료형(클래스) - 괄호로 변환 불가
...
class TheOne {
	...
	public void setStr(String str) {		// (): 매개변수, 사용하는 곳에서 전달해줘야하는 값을 지정하는 변수들
		this.str = str;			// this. : 이 클래스의, 멤버에 있는 변수나 메서드를 사용하기 위해 앞에 붙여
	}
}

interface TheTwo {
	public void go();		// 인터페이스의 메서드는 상속받은 곳에서 {}을 구현해줘야 함
}